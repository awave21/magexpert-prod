...
watch(() => props.events, (newEvents) => {
  // Сохраняем текущие события и добавляем новые, если это загрузка дополнительной страницы
  if (newEvents.current_page > 1) {
    localEvents.value = [...localEvents.value, ...newEvents.data];
  } else {
...
const loadMoreEvents = () => {
  ...
  router.get(route('events.index'), params, {
    ...
    onSuccess: (page) => {
      ...
      localEvents.value = [...localEvents.value, ...page.props.events.data];
...
const remainingSpeakers = computed(() => {
  ...
  const remaining = props.speakersCount - (props.randomSpeakers.length >= 3 ? 3 : props.randomSpeakers.length);
...
const remainingSpeakersText = computed(() => {
  ...
  return `+${remainingSpeakers.value} ${declensionExperts(remainingSpeakers.value)}`;
});
                    ...
                    <ol class="flex items-center space-x-1">
                      ...
                      <li>
                        <div class="flex items-center">
                          ...
                          <span class="ml-1 font-medium text-white" aria-current="page">Мероприятия</span>
                        </div>
                      </li>
                    </ol>
                  </nav>
                  
                  <h1 class="mt-4 text-3xl font-bold tracking-tight text-white sm:text-4xl">Мероприятия</h1>
                  <p class="mt-4 text-lg text-white/90">
                    Вебинары, курсы и конференции для медицинских специалистов. Регистрируйтесь на ближайшие события или смотрите прошедшие в записи.
                  </p>
                </div>
                ...
                <div class="flex items-center justify-center md:justify-end">
                  <div class="flex items-center">
                    <div class="flex -space-x-2 mr-3">
                      <template v-for="(speaker, index) in randomSpeakers" :key="speaker.id">
                        <div v-if="index < 3" class="inline-block h-16 w-16 rounded-full border-2 border-white bg-gray-100 overflow-hidden">
                          <img v-if="speaker.photo" :src="speaker.photo" :alt="`${speaker.first_name} ${speaker.last_name}`" class="h-full w-full object-cover" />
                          <div v-else class="flex h-full w-full items-center justify-center bg-brandblue/20 text-white text-lg font-medium">
                            {{ speaker.first_name[0] }}{{ speaker.last_name[0] }}
                          </div>
                        </div>
                      </template>
                    </div>
                    <span v-if="remainingSpeakersText" class="font-semibold text-white text-lg whitespace-nowrap">
      ...
      <div class="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
          ...
          <div class="block md:hidden">
            ...
            <div class="mb-3">
              <div class="bg-white inline-flex w-full rounded-full p-1 ring-1 ring-inset ring-gray-300 dark:bg-gray-800 dark:ring-gray-700 h-12">
                <button 
                  @click="switchTab('all')" 
                  :class="[
                    activeTab === 'all' 
          ...
          <div class="hidden md:block">
            <div class="flex flex-col space-y-4 md:flex-row md:items-center md:space-y-0 md:space-x-4">
              ...
              <div class="flex items-center space-x-2 flex-shrink-0">
                <div class="bg-white inline-flex rounded-full p-1 ring-1 ring-inset ring-gray-300 dark:bg-gray-800 dark:ring-gray-700 h-12">
                  <button 
                    @click="switchTab('all')" 
                    :class="[
                      activeTab === 'all' 
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <p class="text-sm text-gray-600 dark:text-gray-400">
            Найдено <span class="font-medium text-gray-900 dark:text-white">{{ eventCount }}</span> мероприятий
          </p>
          <div class="mt-2 sm:mt-0">
            ...
            >
              <option value="start_date_asc">По дате (сначала новые)</option>
              <option value="start_date_desc">По дате (сначала старые)</option>
        ...
        <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          <EventCard v-for="event in localEvents" :key="event.id" :event="event" />
        </div>
        ...
        <div v-if="!hasMorePages && !loadingMore && localEvents.length > 0" class="mt-8 text-center text-gray-500 dark:text-gray-400">
